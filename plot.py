...
class Plot:
    fig = None
    additional_traces = {}
    def __init__(self, data: DataFrame, name: str, show_profit: bool = True, dark_mode: bool = True):
    ...
    def bar(self, col):
        ...
        marker_colors[(self.data[col] > self.data[col].shift()) & (self.data[col] > 0)] = "green"
        marker_colors[(self.data[col] > self.data[col].shift()) & (self.data[col] < 0)] = "RGBA(255, 0, 0, .25)"
        marker_colors[self.data[col] == self.data[col].shift()] = "grey"
        marker_colors[(self.data[col] < self.data[col].shift()) & (self.data[col] > 0)] = "RGBA(0, 255, 0, .25)"
        marker_colors[(self.data[col] < self.data[col].shift()) & (self.data[col] < 0)] = "red"
    ...
    def __setup_fig(self):
        ...
        self.fig.update_layout(template="plotly_dark" if self.dark_mode else "plotly_white", hovermode="x", spikedistance=-1)
