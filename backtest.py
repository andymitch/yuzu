...
def backtest(data, interval, stop_loss=.35, trading_fee=.001, verbose=False, respond=True):
    ...
    data['stoped_loss'] = [None] * len(data)
    ...
    for i, row in data.iterrows():
        if not isnan(row['buy']) and wallet['base'] > 0:
            ...
            if verbose:
                colorprint(i, 'buy', row['buy'], interval[-1] != 'd')
            ...
            stop_loss_value = None
            if verbose:
                colorprint(i, 'sell', row['sell'], interval[-1] != 'd')
